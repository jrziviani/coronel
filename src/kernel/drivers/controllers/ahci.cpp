...
lib::unique_ptr<block_device> ahci::detect(pci_info_t info)
{
    ...
    return lib::unique_ptr<block_device>(new ahci::ahci_controller(abar));
}
...
klib::unique_ptr<char[]> ahci::ahci_controller::read(size_t lba, size_t bytes)
{
    ...
    if (sector_size_ == 0) {
        // "sector size == 0, device is not initialized"
        return nullptr;
    }

    // create a buffer large enough to store the result
    auto buffer = make_unique<char[]>(bytes);
                                         ...
                                         uintptr_t buffer, size_t buffer_size)
{
    ...
    auto len = min(buffer_size, PRD_MAX_DATA_LENGTH) / cmd_list->prdt_length;
