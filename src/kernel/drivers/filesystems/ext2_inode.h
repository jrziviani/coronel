...
enum filesystem_states {
    CLEAN = 1,
    DIRTY = 2,
};

enum error_behaviors {
    IGNORE = 1,
    REMOUNT = 2,
    PANIC = 3,
};
...
struct inode {                          //                (4 bits)       (botton 12 bits) 
    ...
    uint32_t osd1[2];                   // OS-specific data (see os_specific_*)
    ...
    uint32_t osd2[3];                   // OS-specific data (see os_specific_*)
};

enum directory_entry_types {
